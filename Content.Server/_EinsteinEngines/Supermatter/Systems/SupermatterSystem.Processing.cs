    ...
    public DelamType ChooseDelamType(EntityUid uid, SupermatterComponent sm)
    {
        ...
        if (mix is { })
        {
            ...
            if (_config.GetCVar(SMCCVars.SupermatterDoSingulooseDelam)
                && moles >= sm.MolePenaltyThreshold * _config.GetCVar(SMCCVars.SupermatterSingulooseMolesModifier))
                return DelamType.Singulo;
        }

        if (_config.GetCVar(SMCCVars.SupermatterDoTeslooseDelam)
            && sm.Power >= sm.PowerPenaltyThreshold * _config.GetCVar(SMCCVars.SupermatterTesloosePowerModifier))
