...
public sealed class CosmicMonumentSystem : EntitySystem
{
    ...
    [Dependency] private readonly TurfSystem _turf = default!; // L5 - turf helpers is kill
    ...
    private bool VerifyPlacement(Entity<CosmicCultLeadComponent> uid, out EntityCoordinates outPos)
    {
        ...
        {
            if (_turf.IsSpace(tile)) // L5 - turf helpers is kill
            {
        ...
        if (TryComp<StationDataComponent>(station, out var stationData))
            stationGrid = _station.GetLargestGrid((station.Value, stationData)); // L5 - station API changes
